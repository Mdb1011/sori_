...
nam = "u0DdN8v0e9556f0184f701eab867daac"
		...
		for msg in messages:
			...
			if msg["type"]=="Text" and not msg.get("message_id") in answered:
				...
				if not sleeped:
					...
					if hasInsult(msg.get("text"))[0] and not msg.get("author_object_guid") in admins :
					...
					elif "from" in msg.keys() and bot.getMessagesInfo(target, [msg.get("message_id")])[0]["forwarded_from"]["type_from"] == "Channel" and not msg.get("author_object_guid") in admins :
						...
						bot.sendMessage(target, "پیام با موفقیت پاک شد✅", message_id=msg.get("message_id"))
						...
						gui = msg.get("author_object_guid")

						esm = bot.getUserInfo(gui)["data"]["user"]["first_name"]

						bot.sendMessage(target, "سلام "+esm+' 😁', message_id=msg.get("message_id"))
						...
						bot.sendMessage(target, "هنو تو شوهر نکردی؟", message_id=msg.get("message_id"))

					elif msg.get("text") == "😡" and msg.get("author_object_guid") :

						bot.sendMessage(target, "گوجه کیلو چند؟", message_id=msg.get("message_id"))

					elif msg.get("text") == "خاطره" and msg.get("author_object_guid") :
						
						res= requests.get("http://api.codebazan.ir/jok/khatere").text
						
						bot.sendMessage(target, "🎭"+res, message_id=msg.get("message_id"))
					elif msg.get("text") == "حدیث" and msg.get("author_object_guid") :
						
						res= requests.get("http://api.codebazan.ir/hadis/").text
						
						bot.sendMessage(target, "🎃"+res, message_id=msg.get("message_id"))
					
					elif msg.get("text") == "-search" and msg.get("author_object_guid") :
						dl=msg.get('text').split()[1]
						res = requests.get("http://api.codebazan.ir/wiki/?search="+dl).text
						bot.sendMessage(msg.get("author_object_guid"), res)
						bot.sendMessage(target, "نتایج به پیوی شما ارسال خواهد شد✅", message_id=msg.get("message_id"))
					    ...
					    k=msg.get('text').replace("-bego","")
					    modir1="u0DdN8v0e9556f0184f701eab867daac"
					    modir="u0D5lXz0bcc92aa5b6b65c858c3d9fec"
					
					    if msg.get("author_object_guid") ==modir:
						    bot.sendMessage(target, k)
					    else:
						    bot.sendMessage(target, "متاسفم گلم شما گدرت زیادی ندارید😇‌\n\nفقط من حرف سازندم رو گوش میدم😁",message_id=msg.get("message_id"))
					elif msg["text"].startswith("-block") and msg.get("author_object_guid") in admins :

						dl=msg.get('text').split()[1]

						ok = bot.getInfoByUsername(dl)['data']['user']['user_guid']

						try:

							bot.block(ok)

							bot.sendMessage(target, 'کاربـــــــر بــــلاک شـــــد✔', message_id=msg["message_id"])

						except:

							bot.sendMessage(target, "❌", message_id=msg["message_id"])

					elif msg["text"].startswith("-unblock") and msg.get("author_object_guid") in admins :
						...
						try:
							...
							bot.sendMessage(target, 'کـــاربر از حالت بلاک خارج شد✔', message_id=msg["message_id"])
						...
						try:
							...
							bot.sendMessage(target, "🔷ترجمه شما:\n"+li.json()['result'], message_id=msg["message_id"])
						...
						bot.sendMessage(target, "https://t.me/proxy?server="+hs+"&port="+hd+"&secret="+hj+'‌\n\nلینک پروکسی تلگرام 📍', message_id=msg["message_id"])
						...
						try:
							response = requests.get(f"https://api.codebazan.ir/font/?text={msg.get('text').split()[1]}").json()

							bot.sendMessage(msg.get("author_object_guid"), "\n".join(list(response["result"].values())[:100])).text
				...
				if data["type"]=="RemoveGroupMembers":
					...
					user = bot.getUserInfo(data['peer_objects'][0]['object_guid'])["data"]["user"]["first_name"]
					time = requests.get('http://api.codebazan.ir/time-date/?td=date').text
					fd = requests.get('http://api.codebazan.ir/time-date/?td=time').text
					bot.sendMessage(target, f"🍄سلام {user} عزیز به گروه {name} خوش اومدید.‌\n\n"+'📅 تاریخ ورود:'+time+'\n⏰ساعت:'+fd,message_id=msg["message_id"])
